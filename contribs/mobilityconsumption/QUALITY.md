# Mobility consumption quality harness

The Mobility consumption quality harness gives contributors one local gate for formatting,
...
- Java 25
- Maven 3.8 or newer
- bash 3.2 or newer (the scripts avoid bash 4 builtins so macOS's stock shell works)
- Git, when using the pre-commit hook

Run the complete gate from anywhere in the repository:

```bash
contribs/mobilityconsumption/scripts/quality.sh
```
...
scripts pass only Java files changed from `origin/main` (or `upstream/main` when the fork has no `main`; override with `QUALITY_BASE_REF`) to Spotless: existing
...
```bash
contribs/mobilityconsumption/scripts/format.sh
```
...
```bash
contribs/mobilityconsumption/scripts/install-pre-commit-hook.sh
```
...
make that hook invoke `contribs/mobilityconsumption/scripts/pre-commit`.

The hook skips commits without relevant MC changes. Maven reads the working
...
```bash
contribs/mobilityconsumption/scripts/quality-metrics.sh --check
```

When metrics improve, ratchet the committed baseline upward with:

```bash
contribs/mobilityconsumption/scripts/quality-metrics.sh --ratchet
```
