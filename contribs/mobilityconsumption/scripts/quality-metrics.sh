...
BASELINE_FILE=${MC_METRICS_BASELINE:-$MODULE_DIR/quality/metrics-baseline.properties}
TARGET_DIR=${MC_METRICS_TARGET:-$MODULE_DIR/target}
...
fail_missing() {
	printf 'MC metrics: required report is missing: %s\n' "$1" >&2
...
read_attribute_total() {
	...
	for file in "$SUREFIRE_DIR"/TEST-*.xml; do
		...
		[[ "$value" =~ ^[0-9]+$ ]] || {
			printf 'MC metrics: cannot read %s from %s.\n' "$attribute" "$file" >&2
			exit 2
		}
		total=$((total + value))
		found=1
...
baseline_value() {
	...
	[[ "$value" =~ ^[0-9]+$ ]] || {
		printf 'MC metrics: baseline %s is missing or invalid in %s.\n' "$key" "$BASELINE_FILE" >&2
		exit 2
	}
	printf '%d\n' "$value"
}
...
coverage=$(awk -F, 'NR > 1 { lm += $8; lc += $9; bm += $6; bc += $7 }
	...
	printf 'MC metrics: cannot calculate coverage from %s.\n' "$JACOCO_CSV" >&2
	exit 2
}
...
if ((regressions > 0)); then
	printf '\nMC metrics gate failed with %d regression(s).\n' "$regressions" >&2
	printf 'Restore the metric or improve it beyond the committed baseline. Never lower the baseline.\n' >&2
	exit 1
fi

if [[ "$mode" == --ratchet ]]; then
	temporary=$(mktemp "${TMPDIR:-/tmp}/mc-metrics-baseline.XXXXXX") || exit 2
	{
		printf '# Monotonic MC quality baseline. Update only with scripts/quality-metrics.sh --ratchet.\n'
		for key in "${METRIC_KEYS[@]}"; do
			printf '%s=%d\n' "$key" "${!key}"
		done
	} >"$temporary"
